...
from tkinter import END, Tk, StringVar
...
def split_drawings():
    ...
    for i, filename in enumerate(pdf_files, start=1):
        ...
        drawing_nums = drawing_splitter.get_dwg_info(filename, num_of_pages,
                                                     number_element,
                                                     regions[REGIONS[region]],
                                                     revision, progress_bar)
        drawing_splitter.save_drawings(filename, num_of_pages, drawing_nums,
...
label_input_folder = Label(master=input_frame, text='Input Folder:',
                           font=('Calibri', 10, 'bold'))
...
entry_input_folder = Entry(master=input_frame, state='disabled', width=50,
                           textvariable=string_input_folder)
entry_input_folder.grid(row=1, column=1, pady=5, columnspan=4, sticky='nsew')
button_input_folder = Button(master=input_frame, text='Browse...',
                             command=lambda: browsefunc(entry_input_folder))
...
label_output_folder = Label(master=input_frame, text='Output Folder:',
                            font=('Calibri', 10, 'bold'))
...
entry_output_folder = Entry(master=input_frame, state='disabled', width=50,
                            textvariable=string_output_folder)
entry_output_folder.grid(row=3, column=1, pady=5, columnspan=4, sticky='nsew')
button_output_folder = Button(master=input_frame, text='Browse...',
                              command=lambda: browsefunc(entry_output_folder))
...
label_delete = Label(master=input_frame, text='Delete files:',
                     font=('Calibri', 10, 'bold'))
...
option_delete = Checkbutton(input_frame, variable=string_delete,
                            onvalue=True, offvalue=False)
option_delete.grid(row=5, column=1, sticky='w', padx=5)

# Drop down box for saving by revision
label_revision = Label(master=input_frame, text='Save by revision:',
                       font=('Calibri', 10, 'bold'))
...
option_revision = Checkbutton(input_frame, variable=string_revision,
                              onvalue=True, offvalue=False)
option_revision.grid(row=5, column=3, sticky='w', padx=5)

# Drop down box for region
label_region = Label(master=input_frame, text='Region:',
                     font=('Calibri', 10, 'bold'))
...
button_start = Button(master=input_frame, text='Split!',
                      command=save_and_split)
button_start.grid(row=7, column=5, sticky="e")


# Add progress bar
progress_bar = Progressbar(master=input_frame, orient='horizontal',
                           length=490, mode='determinate')
progress_bar.grid(row=8, column=0, columnspan=6, pady=5)

# Status label
label_status = Label(master=input_frame, text='Status:',
                     font=('Calibri', 10, 'bold'))
